...
添加集成软件包
`cfdisk, htop, openssl-util, kmod-fuse, kmod-usb-net-ipheth, kmod-usb-net-rndis, luci-app-adguardhome, luci-app-ddns-go, luci-app-frps, luci-app-argon-config, luci-app-autoreboot, luci-app-diskman, luci-app-ksmbd, luci-app-nlbwmon, luci-app-openclash, luci-app-openvpn, luci-app-ttyd, luci-app-upnp, luci-app-usb-printer, luci-app-zerotier, luci-app-ttyd, luci-app-wol, luci-theme-argon`
等并预置 openclash 内核
...
### 配置说明
- 默认 LAN IP 已更改为 `10.0.0.1`，可在 `scripts/diy.sh` 处修改

- 需要取消集成或添加其他软件包可在 `configs/rax3000m-nand_mtksdk.config` 处参考注释内容自行修改或添加配置选项
